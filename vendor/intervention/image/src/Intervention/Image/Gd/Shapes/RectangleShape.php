...
class RectangleShape extends \Ttc\Freebies\Intervention\Image\AbstractShape
{
    /**
    ...
    {
        $background = new \Ttc\Freebies\Intervention\Image\Gd\Color($this->background);
        imagefilledrectangle($image->getCore(), $this->x1, $this->y1, $this->x2, $this->y2, $background->getInt());

        if ($this->hasBorder()) {
            $border_color = new \Ttc\Freebies\Intervention\Image\Gd\Color($this->border_color);
