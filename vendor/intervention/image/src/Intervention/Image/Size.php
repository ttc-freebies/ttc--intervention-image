...
use Closure;
use Ttc\Freebies\Intervention\Image\Exception\InvalidArgumentException;
...
class Size
{
     ...
     * @param Point $pivot
     */
    public function __construct($width = null, $height = null, \Ttc\Freebies\Intervention\Image\Point $pivot = null)
    {
        $this->width = is_numeric($width) ? intval($width) : 1;
        $this->height = is_numeric($height) ? intval($height) : 1;
        $this->pivot = $pivot ? $pivot : new \Ttc\Freebies\Intervention\Image\Point;
    }
     ...
     * @param Point $point
     */
    public function setPivot(\Ttc\Freebies\Intervention\Image\Point $point)
    {
        $this->pivot = $point;
    }
    ...
    public function resize($width, $height, Closure $callback = null)
    {
        if (is_null($width) && is_null($height)) {
            throw new \Ttc\Freebies\Intervention\Image\Exception\InvalidArgumentException(
    ...
    private function resizeWidth($width, Closure $callback = null)
    {
        ...
        if ($constraint->isFixed(\Ttc\Freebies\Intervention\Image\Constraint::UPSIZE)) {
        ...
        if (is_numeric($width)) {
            ...
            if ($constraint->isFixed(\Ttc\Freebies\Intervention\Image\Constraint::ASPECTRATIO)) {
     ...
     *
     * @param  Size   $size
     * @return \Ttc\Freebies\Intervention\Image\Point
     */
    public function relativePosition(\Ttc\Freebies\Intervention\Image\Size $size)
    {
        $x = $this->pivot->x - $size->pivot->x;
        $y = $this->pivot->y - $size->pivot->y;

        return new \Ttc\Freebies\Intervention\Image\Point($x, $y);
    }
     ...
     */
    public function fit(\Ttc\Freebies\Intervention\Image\Size $size, $position = 'center')
    {
     ...
     * @return boolean
     */
    public function fitsInto(\Ttc\Freebies\Intervention\Image\Size $size)
    {
     ...
     *
     * @param  Closure $callback
     * @return \Ttc\Freebies\Intervention\Image\Constraint
     */
    private function getConstraint(Closure $callback = null)
    {
        $constraint = new \Ttc\Freebies\Intervention\Image\Constraint(clone $this);
